...
import {getAlbum, getArtist, searchForAlbum} from '../api/spotify.ts';
...
export default function SpotifySearch({sdk}: { sdk: SpotifyApi | null }) {
    ...
    const onSubmit: SubmitHandler<IFormInput> = (data) => {
        (async () => {
            const searchResults = await searchForAlbum(sdk, data.searchText)
            if (searchResults) {
                setResults(await Promise.all(searchResults.albums?.items.map(async (simplifiedAlbum) => {
                    const artist = await getArtist(sdk, simplifiedAlbum.artists[0].id)
                    const album = await getAlbum(sdk, simplifiedAlbum.id)
                    return {
